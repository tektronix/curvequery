...
def read_binary_values_with_custom_read_methods(_raw_read_method, read_bytes_method):
    def modified_read_binary_values(
        self,
        ...
        """
        block = _raw_read_method(chunk_size)
        ...
        if data_length != 0:
            block.extend(
                read_bytes_method(expected_length - len(block), chunk_size=chunk_size)
            )
        else:
            raise ValueError(
    ...
    return modified_read_binary_values


def _raw_read_with_tqdm(tqdm_obj):
    def modified_read_raw(self, size: Optional[int] = None):
